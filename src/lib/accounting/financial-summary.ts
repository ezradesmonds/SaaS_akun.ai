...
export function isCashAccount(account: CashAccount) {
  return (
    account.type === 'ASSET' &&
    (['1-001', '1-002'].includes(account.code) ||
      /^(kas|bank)(\s|$)/i.test(account.name))
  )
}
...
export function summarizeCash(lines: CashLine[], start: string, end: string) {
  ...
  for (const line of lines) {
    if (
      !line.account ||
      !isCashAccount(line.account) ||
      line.transaction.date > end
    )
      continue
    ...
    if (line.transaction.date < start) opening += amount
      ...
      movements.set(
        line.transaction.id,
        (movements.get(line.transaction.id) || 0) + amount,
      )
  }
  ...
  return {
    ...
    cash_balance: opening + cashIn - cashOut,
  }
}

export function unclosedEarnings(
