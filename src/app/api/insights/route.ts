...
async function respond(request: NextRequest, reason: boolean) {
  ...
  if (!ctx.can('view_reports') || (reason && !ctx.can('use_ai_chat')))
    return NextResponse.json(AUTH_ERRORS.forbidden, { status: 403 })
  if (reason && !ctx.withinLimit('ai'))
    return NextResponse.json(AUTH_ERRORS.plan_limit_ai, { status: 402 })
  try {
    ...
    return NextResponse.json(
      {
        error:
          'Ringkasan belum dapat dimuat. Coba lagi dan pastikan data bisnis tersedia.',
      },
      { status: 503 },
    )
  }
